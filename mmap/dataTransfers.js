 ...
 *
 *    NOTE: This applet is no longer functional as the resource it 
 *          makes requests to has been taken offline.
 */
...
function ISolemnlySwearIAmUpToNoGood(myPos) {
	...
	marker = new google.maps.Marker({ position: myPos, title: login, icon: "legend-of-zelda.png"});
	marker.setMap(map);
...
function addMarker(peer) {
		var pos, peerMarker, distance;
		
		pos = new google.maps.LatLng(peer.lat, peer.lng);
		peerMarker = new google.maps.Marker({position: pos, title: peer.login});
		peerMarker.setMap(map);
		distance = haversine(peer);
